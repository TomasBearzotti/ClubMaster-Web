  ...
  const loadCuotas = async () => {
    try {
      ...
      if (response.ok) {
        ...
        const montoTotal = cuotasData.reduce((sum: number, c: any) => sum + Number.parseFloat(c.Monto), 0)
        const montoPagado = cuotasData
          .filter((c: any) => c.Estado === 1)
          .reduce((sum: number, c: any) => sum + Number.parseFloat(c.Monto), 0)
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-gray-100">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-8">
          ...
          <p className="text-gray-600">Administra las cuotas mensuales de los socios del club</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{estadisticas.totalCuotas}</div>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold text-green-600">{estadisticas.cuotasPagadas}</div>
              <p className="text-xs text-muted-foreground">
                {estadisticas.totalCuotas > 0
                  ? Math.round((estadisticas.cuotasPagadas / estadisticas.totalCuotas) * 100)
                  : 0}
                % del total
              </p>
            </CardContent>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold text-yellow-600">{estadisticas.cuotasPendientes}</div>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold text-red-600">{estadisticas.cuotasVencidas}</div>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="text-center">
                <div className="text-2xl font-bold text-blue-600">{formatCurrency(estadisticas.montoTotal)}</div>
                <div className="text-sm text-gray-600">Monto Total Esperado</div>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold text-green-600">{formatCurrency(estadisticas.montoPagado)}</div>
                <div className="text-sm text-gray-600">Monto Recaudado</div>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold text-orange-600">
                  {formatCurrency(estadisticas.montoTotal - estadisticas.montoPagado)}
                </div>
          ...
          <CardHeader>
            <div className="flex justify-between items-center">
              <div>
                ...
                <CardDescription>Lista de todas las cuotas del sistema</CardDescription>
              </div>
