...
export async function POST(request: NextRequest) {
  try {
    ...
    if (tipoFixture === undefined || tipoFixture === null) {
      return NextResponse.json(
        { error: "tipoFixture es requerido" },
        { status: 400 }
      );
    }
    ...
    if (isNaN(tipoTorneo) || tipoTorneo < 0 || tipoTorneo > 2) {
      return NextResponse.json(
        {
          error:
            "tipoFixture inválido. Valores permitidos: 0 (Liga), 1 (Eliminación), 2 (Grupos+Eliminación)",
        },
        { status: 400 }
      );
    }
    ...
    if (fixtureExistente.recordset[0].Total > 0) {
      return NextResponse.json(
        {
          error:
            "Ya existe un fixture para este torneo. Elimina el fixture existente antes de generar uno nuevo.",
        },
        { status: 400 }
      );
    }
    ...
    if (participantes.length < 2) {
      return NextResponse.json(
        {
          error:
            "Se necesitan al menos 2 participantes para generar un fixture",
        },
        { status: 400 }
      );
    }
    ...
    try {
      ...
      await transaction
        ...
        .query(
          `UPDATE Torneos SET TipoTorneo = @tipoTorneo WHERE IdTorneo = @torneoId`
        );
      ...
      for (let i = 0; i < fixtureGenerado.fixtures.length; i++) {
        ...
        const result = await transaction
          ...
          .input("fechaFin", sql.DateTime2, fixture.FechaFin).query(`
      ...
      for (const partido of fixtureGenerado.partidos) {
        ...
        const request = transaction
          ...
          .input(
            "fechaHora",
            sql.DateTime2,
            partido.FechaHora ?? new Date("1900-01-01")
          )
      ...
      return NextResponse.json({
        success: true,
        message: `Fixture generado exitosamente usando estrategia: ${
          fixtureContext.getInfo().nombre
        }`,
