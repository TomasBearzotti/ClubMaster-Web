...
export async function GET(
  ...
  try {
    ...
    if (isNaN(fixtureId)) {
      return NextResponse.json({ error: "ID de fixture inválido" }, { status: 400 })
    }
...
export async function PUT(
  ...
  try {
    const fixtureId = Number.parseInt(params.id)
    const { Nombre, NumeroRonda, Grupo, FechaInicio, FechaFin } = await request.json()
    ...
    await pool
      ...
      .input("fechaInicio", sql.DateTime2, FechaInicio ? new Date(FechaInicio) : null)
