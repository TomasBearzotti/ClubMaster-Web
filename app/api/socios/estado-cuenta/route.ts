...
export async function GET(request: NextRequest) {
  try {
    ...
    const cuotasVencidas = cuotasVencidasResult.recordset[0].CuotasVencidas || 0
    const deudaBase = cuotasVencidasResult.recordset[0].DeudaBase || 0
    const deudaConRecargo = cuotasVencidasResult.recordset[0].DeudaConRecargo || 0
    ...
    if (cuotasVencidas >= 6) {
      ...
      mensajeAlerta = "Tu membresía ha sido suspendida por tener 6 o más cuotas vencidas"
